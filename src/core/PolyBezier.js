export class PolyBezier {
  constructor(beziers) {
    ...
    this.length = this.beziers.reduce((acc, current) => acc + current.getLength(), 0);
    this.lengths = this.beziers.map((bezier) => bezier.getLength() / this.length);
  ...
  toSVG() {
    return this.beziers.reduce((acc, current) => {
